...
export const label = (value: string | null | undefined) => value === 'ESCALATE_COMMERCIAL_CONSTRAINT' ? 'Review commercial trade-off' : value === 'PREBOOK_CAPACITY' ? 'Pre-book capacity' : value === 'PRICE_COMPETITIVENESS' ? 'Pricing competitiveness' : value ? value.toLowerCase().replaceAll('_', ' ').replace(/^./, c => c.toUpperCase()) : 'None'
