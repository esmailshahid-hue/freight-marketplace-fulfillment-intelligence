...
export async function requestOperationsBrief(analysis: Analysis, asOf: string, currency: string, signal?: AbortSignal, fetcher: typeof fetch = fetch) {
  const context = buildBriefContext(analysis, asOf, currency)
