...
export function SupplyGaps({ gaps, currency }: { gaps: Analysis['supply_gaps']; currency: string }) {
  ...
  return <>
    ...
    <div className="supply-leaders">{rows.slice(0, 3).map(r => <article key={r.rank}>
      <div className="attention-top"><span className="rank-label">{String(r.rank).padStart(2, '0')}</span><span className="muted">{r.equipment_type}</span></div>
      <h3 className="lane-title">{r.lane}</h3><strong className="supply-score">{number(r.structural_supply_gap_score, 0)}</strong><small>Supply-gap priority score</small>
