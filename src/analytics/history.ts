...
export function demandBaseline(history: HistoricalLoad[], upcoming: UpcomingLoad[], lane: string, equipment: string, asOf: string, timeZone = DEFAULT_TIME_ZONE) {
  ...
  const today = dateNumber(endDate), weekday = new Date(today).getUTCDay()
  ...
  const now = timestamp(asOf, timeZone)
  const weekly = sum(upcoming.filter(r => r.lane === lane && r.equipment_type === equipment && timestamp(r.pickup_datetime, timeZone) >= now && timestamp(r.pickup_datetime, timeZone) <= now + 7 * DAY), r => r.load_count)
