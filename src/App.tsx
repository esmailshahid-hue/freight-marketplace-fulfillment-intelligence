...
import './App.css'
interface Snapshot { data: Datasets; analysis: Analysis; asOf: string; warnings: DataIssue[] }
...
function Workspace({ snapshot }: { snapshot: Snapshot }) {
  ...
  const { analysis, data, asOf, warnings } = snapshot
  const currency = data.upcoming[0]?.currency ?? 'SAR'
  ...
  return <>
    <div className="data-context"><p><strong>Built-in synthetic sample</strong> · Seven-day baseline · {currency}</p>
    ...
    <details className="data-status"><summary>Data status: sample loaded · {warnings.length} validation warnings</summary>
      <p>{data.historical.length} historical loads · {data.offers.length} offers · {data.capacity.length} capacity blocks · {data.payments?.length ?? 0} carrier payment rows. Dates shifted to this analysis snapshot.</p>
      {warnings.length ? <ul>{warnings.map((warning, i) => <li key={i}>{warning.dataset}{warning.row ? `, row ${warning.row}` : ''}: {warning.message}</li>)}</ul> : <p>No validation warnings.</p>}
    </details>
    ...
    <main id="main-content">{TABS.map((name, i) => <div role="tabpanel" id={`panel-${i}`} aria-labelledby={`tab-${i}`} hidden={tab !== name} key={name}>
      ...
      {name === 'Pricing & Economics' && <PricingEconomics buckets={analysis.buckets} onBucket={openBucket} />}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setState({ status: 'ready', snapshot: { data: loaded.data, analysis, asOf, warnings: loaded.warnings } })
      } catch (error) {
  ...
  return <div className="app-shell">
    ...
    <header className="app-header"><p className="eyebrow">Phase 2A · Operations workspace</p>
    ...
    {state.status === 'ready' && <Workspace snapshot={state.snapshot} />}
